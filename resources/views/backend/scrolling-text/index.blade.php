...
@section('content')
                    ...
                    <div class="card-body">
                        <form action="{{ route($base_route . 'store') }}" method="POST" enctype="multipart/form-data"
                            ...
                            <div class="col-md-12">
                                    ...
                                    <label for="floatingName">Add Scrolling Text</label>
                        ...
                        <div class="card-body">
                            <form action="{{ route($base_route . 'update', $post->id) }}" method="POST"
                                ...
                                <div class="col-md-12">
                                        ...
                                        <textarea name="description" type="text" class="form-control description">
                                            {{ $post->description }}
                                        </textarea>
    ...
    <div class="col-12">
                ...
                <div class="datatable-wrapper datatable-loading no-footer sortable searchable fixed-columns">
                    ...
                    <div class="datatable-container mt-2">
                        <table class="table table-borderless datatable datatable-table">
                            ...
                            <tbody>
                                @foreach ($data['posts'] as $scrolling)
                                    <tr data-index="0">
                                        <td> {{ $loop->iteration }} </td> 
                                        
                                        <td><small>{{ $scrolling->description }}</small></td>
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input check-uncheck"
                                                    data-id="{{ $scrolling->id }}" type="checkbox" role="switch"
                                                    id="flexSwitchCheckChecked"
                                                    {{ $scrolling->status == '0' ? 'checked' : '' }}>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-around">
                                                <a href="#" data-bs-toggle="modal"
                                                    data-bs-target="#viewPost-{{ $scrolling->id }}">
                                                    <i class="bi bi-eye-fill fs-5 p-2 text-success"></i></a>
                                                <a href="#" data-bs-toggle="modal"
                                                    data-bs-target="#editPost-{{ $scrolling->id }}">
                                                    <i class="bi bi-pencil-square fs-5 p-2"></i></a>

                                                <form action="{{ route($base_route . 'destroy', $scrolling->id) }}"
        ...
        {{-- Post View  --}}
        @foreach ($data['posts'] as $scrolling)
            <div class="modal fade" id="viewPost-{{ $scrolling->id }}" tabindex="-1" aria-labelledby="viewPostLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-1 row">
                                ...
                                <div class="col-sm-6">
                                    <small>{{ $scrolling->description ?? '' }}</small>
                                </div>
                            </div>
                            ...
                            <div class="mb-1 row">
                                ...
                                <div class="col-sm-4 mt-2">
                                    <small>{{ $scrolling->createdBy?->name ?? '' }}</small>
                                </div>
                                <label for="inputPassword" class="col-sm-2 col-form-label">Updated by</label>
                                <div class="col-sm-4 mt-2">
                                    <small>{{ $scrolling->updatedBy?->name ?? 'NULL' }}</small>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            $('.check-uncheck').click(function() {
                ...
                $.ajax({
                    url: "{{ route('scrolling_text.status') }}",
                    data: {
                        id: id
                    },
