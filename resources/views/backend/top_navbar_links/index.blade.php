...
@section('content')
                                ...
                                <div class="col-md-5">
                                    <div class="form-floating">
                                        <input type="text" name="name" class="form-control name"
                                            value="{{ $post->name }}" id="floatingName" placeholder="Title">
                                        <label for="floatingName">Title</label>
                                    </div>
                                </div>
    ...
    <div class="col-12">
                ...
                <div class="datatable-wrapper datatable-loading no-footer sortable searchable fixed-columns">
                    ...
                    <div class="datatable-container mt-2">
                        <table class="table table-borderless datatable datatable-table">
                            ...
                            <tbody>
                                @foreach ($data['posts'] as $topnavbarlinks)
                                    <tr data-index="0">
                                        <td> {{ $loop->iteration }} </td>
                                        <td>
                                            <small> {{ $topnavbarlinks->name }} </small>
                                        </td>
                                        <td><small>{{ $topnavbarlinks->link }}</small></td>

                                        <td><small>{{ $topnavbarlinks->rank }}</small></td>

                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input check-uncheck"
                                                    data-id="{{ $topnavbarlinks->id }}" type="checkbox" role="switch"
                                                    id="flexSwitchCheckChecked"
                                                    {{ $topnavbarlinks->status == '0' ? 'checked' : '' }}>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-around">
                                                <a href="#" data-bs-toggle="modal"
                                                    data-bs-target="#viewPost-{{ $topnavbarlinks->id }}">
                                                    <i class="bi bi-eye-fill fs-5 p-2 text-success"></i></a>
                                                <a href="#" data-bs-toggle="modal"
                                                    data-bs-target="#editPost-{{ $topnavbarlinks->id }}">
                                                    <i class="bi bi-pencil-square fs-5 p-2"></i></a>

                                                <form action="{{ route($base_route . 'destroy', $topnavbarlinks->id) }}"
        ...
        {{-- Post View  --}}
        @foreach ($data['posts'] as $topnavbarlinks)
            <div class="modal fade" id="viewPost-{{ $topnavbarlinks->id }}" tabindex="-1"
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-1 row">
                                ...
                                <div class="col-sm-6">
                                    <small>{{ $topnavbarlinks->link ?? '' }}</small>
                                </div>
                            </div>
                            ...
                            <div class="mb-1 row">
                                ...
                                <div class="col-sm-6">
                                    <small>{{ $topnavbarlinks->rank ?? '' }}</small>
                                </div>
                            </div>
                            ...
                            <div class="mb-1 row">
                                ...
                                <div class="col-sm-4 mt-2">
                                    <small>{{ $topnavbarlinks->createdBy?->name ?? '' }}</small>
                                </div>
                                <label for="inputPassword" class="col-sm-2 col-form-label">Created by</label>
                                <div class="col-sm-4 mt-2">
                                    <small>{{ $topnavbarlinks->createdBy?->name ?? '' }}</small>
    ...
    <script>
        $(document).ready(function() {
            $('.check-uncheck').click(function() {
                ...
                $.ajax({
                    url: "{{ route('top_navbar_links.status') }}",
                    data: {
                        id: id
                    },
